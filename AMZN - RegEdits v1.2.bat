@echo off
TITLE AMZN - RegEdits v1.2
:: 
:: =================================================================
:: Script Name: Amazon Download Script
:: Description: This script helps users input and validate various
::              parameters for downloading content from Amazon. 
...
ECHO                                         ====================================
ECHO.
ECHO                                          AMZN Upload Script For VineTrimmer
ECHO.
ECHO                                         ====================================
ECHO.
ECHO.
...
ECHO  ====================================================================================================================
ECHO.
ECHO [Info] This script is designed to facilitate the use of VineTrimmer for downloading videos from Amazon. I have 
ECHO        customized it to enhance user-friendliness according to my standards, making it more accessible and 
ECHO        intuitive for both myself and others. This script does not download content from AMZN directly but rather
...
:URL
SET /p url=Paste Amazon URL Code (or type "exit" to quit): 
IF /I "%url%"=="exit" EXIT
...
IF /I "%codec%"=="H264" GOTO Bitrate
IF /I "%codec%"=="H265" GOTO Bitrate
...
SET /p bitrate=What bitrate would you like? (CBR, CVBR or CBR+CVBR, or type "exit" to quit): 
IF /I "%bitrate%"=="exit" EXIT
IF /I "%bitrate%"=="cbr" GOTO Subtitles
IF /I "%bitrate%"=="cvbr" GOTO Subtitles
IF /I "%bitrate%"=="cbr+cvbr" GOTO Subtitles
ECHO Invalid choice. Please enter CBR, CVBR or CBR+CVBR.
GOTO Bitrate

:Subtitles

:: Checks if value has been set once already to avoid asking questions that are already answered
IF "%BITRATE_SET%"=="1" GOTO LAST_SECTION

:: Set codec flag
IF "%BITRATE_SET%"=="0" SET BITRATE_SET=1
...
:RunRemux
poetry run vt dl --cdm amazon4k -al en-US -q %quality% -v %codec% -r %release% AMZN -b %bitrate% %url%
GOTO End
...
:RunNoRemux
poetry run vt dl --cdm amazon4k -nm -al en-US -q %quality% -v %codec% -r %release% AMZN -b %bitrate% %url%
GOTO End
...
:RunSubtitles
poetry run vt dl --cdm amazon4k -S -al en-US -q %quality% -v %codec% -r %release% AMZN -b %bitrate% %url%
GOTO End
...
IF "%choice%"=="5" GOTO Bitrate
IF "%choice%"=="6" GOTO Subtitles
IF "%choice%"=="7" EXIT
