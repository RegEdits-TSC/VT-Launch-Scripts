@echo off
TITLE DSNP - RegEdits v1.2
:: 
:: =================================================================
:: Script Name: DisneyPlus Download Script
:: Description: This script helps users input and validate various
::              parameters for downloading content from DisneyPlus. 
...
ECHO                                         ====================================
ECHO.
ECHO                                          DSNP Upload Script For VineTrimmer
ECHO.
ECHO                                         ====================================
ECHO.
ECHO.
...
ECHO  ====================================================================================================================
ECHO.
ECHO [Info] This script is designed to facilitate the use of VineTrimmer for downloading videos from DisneyPlus. I have 
ECHO        customized it to enhance user-friendliness according to my standards, making it more accessible and 
ECHO        intuitive for both myself and others. This script does not download content from DSNP directly but rather
...
:URL
SET /p url=Paste DisneyPlus URL (or type "exit" to quit): 
IF /I "%url%"=="exit" EXIT

:: Check if URL starts with the required string
echo %url% | findstr /b /c:"https://www.disneyplus.com/browse/" >nul
IF ERRORLEVEL 1 (
    ECHO Invalid link. The URL should start with "https://www.disneyplus.com/browse/".
    GOTO URL
)
...
:RunRemux
poetry run vt dl --cdm d4k -al en-US -q %quality% -v %codec% -r %release% DSNP -m %url%
GOTO End
...
:RunNoRemux
poetry run vt dl --cdm d4k -nm -al en-US -q %quality% -v %codec% -r %release% DSNP -m %url%
GOTO End
...
:RunSubtitles
poetry run vt dl --cdm d4k -S -al en-US -q %quality% -v %codec% -r %release% DSNP -m %url%
GOTO End
